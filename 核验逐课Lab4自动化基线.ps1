...
foreach ($record in $records) {
    $id = [string]$record.Id
    $dir = Join-Path $evidenceRoot (Join-Path $id 'labs\lab4\automation-baseline')
...
$resultFile = Join-Path $Root '抽样运行_2026-09-09\lab4-automation-results.json'
...
if ($missing.Count -eq 0 -and $badStatus.Count -eq 0 -and $results.Count -eq $records.Count -and $fail -eq 0) {
    Write-Output 'Lab 4 automated baseline: PASS (machine fault-recovery baseline only; learner completion remains separate).'
}
